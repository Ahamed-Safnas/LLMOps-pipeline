    ...
    def __init__(self, required_keys: list[str]):
        ...
        for key in required_keys:
            if not self.api_keys.get(key):
                for env_key in self.KEY_ALIASES.get(key, [key]):
                    env_val = os.getenv(env_key)
                    if env_val:
                        self.api_keys[key] = env_val
                        log.info("Loaded API key from environment", key=key, env_key=env_key)
                        break
...
class ModelLoader:
    ...
    def __init__(self):
        ...
        self.api_key_mgr = ApiKeyManager(self._required_api_keys())

    def _required_api_keys(self) -> list[str]:
        required = set()

        embedding_provider = self.config.get("embedding_model", {}).get("provider")
