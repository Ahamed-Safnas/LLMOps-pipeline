...
class ConversationalRAG:
    ...
    def __init__(self, session_id: Optional[str], retriever=None):
        try:
            ...
            self.llm = self._load_llm().with_retry(stop_after_attempt=3)
    ...
    def _build_lcel_chain(self):
        try:
            ...
            def question_for_retrieval(payload: Dict[str, Any]) -> str:
            ...
            # 1) Retrieve docs directly for the first turn; rewrite only when history exists.
            retrieve_docs = RunnableLambda(question_for_retrieval) | self.retriever | self._format_docs

            # 2) Answer using retrieved context + original input + chat history.
            self.chain = (
                {
